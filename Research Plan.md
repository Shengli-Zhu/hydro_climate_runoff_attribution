...
**Train / validation / test split** (strictly temporal, not random):
- Training: 1950–1996
- Validation: 1997–2004 (used for KGE-based early stopping)
- Testing: 2005–2025 (held-out, ~27% of years)

This three-way temporal ordering prevents data leakage and allows training to be terminated when validation KGE plateaus.
...
xgb.XGBRegressor(
    ...
    early_stopping_rounds=20, eval_metric=neg_kge_eval,
    random_state=42,
)
```

Training is stopped automatically when the validation set KGE has not improved for 20 consecutive boosting rounds.

**Evaluation metrics:**
- **R²**: fraction of variance explained
- **RMSE**: root mean squared error (mm/month)
- **NSE** (Nash-Sutcliffe Efficiency): NSE = 1 − [Σ(obs−sim)² / Σ(obs−mean_obs)²]; equivalent to R² for the test set
...
**Monthly model results** (test period 2005–2025):

| Country | R² | RMSE (mm/month) | NSE | KGE |
|---------|----|-----------------|-----|-----|
| Saudi Arabia | 0.36 | 1.76 | 0.36 | 0.35 |
| Italy | 0.82 | 24.62 | 0.82 | 0.87 |
| Bangladesh | 0.93 | 36.67 | 0.93 | 0.94 |
...
A separate XGBoost model is trained on these annual records with the same temporal split and KGE-based early stopping.

**Annual model results** (test period 2005–2025):

| Country | R² | RMSE (mm/year) | NSE | KGE |
|---------|----|----------------|-----|-----|
| Saudi Arabia | 0.59 | 8.83 | 0.59 | 0.69 |
| Italy | 0.97 | 78.87 | 0.97 | 0.96 |
| Bangladesh | 0.98 | 90.07 | 0.98 | 0.98 |

The substantial improvement in Saudi Arabia (R² 0.36 → 0.59) confirms that annual aggregation removes within-year noise that obscures the climate–runoff relationship in monthly data. **The annual model is the basis for all physical attribution in the main report**; monthly results are retained only as a methodological reference in the Supplementary Information.
...
**Saudi Arabia** (R² = 0.36): The model struggles to predict monthly runoff from monthly climate means. Dewpoint temperature ranks above precipitation as the most "important" feature — a physically implausible result that arises because the model picks up on the seasonal temperature/humidity cycle rather than the episodic precipitation–runoff relationship. This confirms that Saudi Arabia's runoff is driven by stochastic extreme events, and monthly climate averages contain insufficient information.

**Italy** (R² = 0.82): Soil temperature is the top feature (mean |SHAP| ≈ 56 mm/month). This is not directly physical — rather, soil temperature acts as a **seasonal proxy**: low values in winter correspond to low evapotranspiration and snowmelt runoff (positive SHAP), while high summer values correspond to high evapotranspiration suppressing runoff (negative SHAP). The model has learned the seasonal cycle through soil temperature rather than through precipitation.

**Bangladesh** (R² = 0.93): Precipitation is overwhelmingly the top feature (mean |SHAP| ≈ 73 mm/month), with all other features playing secondary roles. The model has correctly learned that month-to-month runoff variation is driven primarily by monsoon rainfall.
...
2. **Model predictability (R²) increases from 0.59 → 0.97 → 0.98**, confirming that in humid climates, annual runoff is almost entirely determined by annual precipitation, while in arid climates, additional factors (extreme event timing, infiltration capacity) introduce irreducible unpredictability.
