...
**Geo Modeling Final Project** | KAUST
...
│   ├── 01_water_balance.py          # Water balance analysis + spatial distribution maps
...
- Saudi Arabia: All features have low SHAP magnitude (|SHAP| < 5 mm/yr); monthly runoff is driven by stochastic extreme events, not monthly climate means
- Italy: Precipitation dominant (|SHAP| ~270 mm/yr); soil temperature important in monthly model (seasonal proxy), recedes in annual model
- Bangladesh: Precipitation overwhelmingly dominant (|SHAP| ~500 mm/yr); antecedent soil water important (saturation feedback)

**Cross-regime gradient**: Runoff predictability (R²) and precipitation dominance both increase systematically from arid to humid, consistent with the shift from water-limited to energy-limited regimes.
...
| fig09/09b | Monthly/annual XGBoost: predicted vs observed |
| fig10/10b | Monthly/annual SHAP temporal evolution |
| fig11/11b | Monthly/annual SHAP beeswarm summary |
| fig12/12b | Monthly/annual feature importance comparison (3 countries) |
| fig13/13b | Monthly/annual precipitation SHAP dependence plot |
